...
void GzCrazyflieInterface::Configure(const gz::sim::Entity &_entity,
	...
	// Unified outbound queue: IMU/baro/odom callbacks and recvCfLib all push here.
	// A single queue lets sendCfFirmwareThread block (wait_dequeue) instead of
	// busy-polling four separate queues. The per-drone busy-spin in the upstream
	// plugin is what drives RTF down once you spawn more than a few drones.
	// Initial capacity 256 absorbs bursts from a 1 kHz IMU when RTF transiently dips.
	m_queueSendCfFirm = moodycamel::BlockingConcurrentQueue<crtpPacket_t>(256);
	firmware_to_cflib_queue = moodycamel::BlockingConcurrentQueue<crtpPacket_t>(256);
...
void GzCrazyflieInterface::sendCfFirmwareThread() {
	...
	while(isPluginOn){
		...
		// Block until data is available (or 100ms passes so we can re-check
		// isPluginOn for clean shutdown). Replaces the four-queue busy-poll
		// that spun a whole CPU core per drone in the upstream plugin.
		size_t msg_count = m_queueSendCfFirm.wait_dequeue_bulk_timed(
			msgs, 16, std::chrono::milliseconds(100));
...
void GzCrazyflieInterface::sendCfLibThread() {
	...
	while(isPluginOn){
		...
		size_t cflib_msg_count = firmware_to_cflib_queue.wait_dequeue_bulk_timed(
			msgs, 16, std::chrono::milliseconds(100));
